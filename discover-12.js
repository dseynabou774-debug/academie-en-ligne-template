// =============================================================================
// discover.js — Espace public "Futurs parents / Découvrir l'académie".
// Accessible SANS profil élève (page publique partageable par
...
function defaultDiscoverContent() {
  return {
    // Ces 4 descriptions insèrent dynamiquement le nom de l'académie
    // (getAcademyName(), lu depuis Réglages) au moment où ce contenu par
    // défaut est généré pour la première fois — plus aucun nom d'académie
    // n'est écrit en dur ici. L'enseignante peut ensuite librement réécrire
    // ce texte via Réglages > Modifier le contenu, sans toucher au code.
    description: {
      fr: `${getAcademyName()} est une académie coranique en ligne qui accompagne enfants et adultes, pas à pas, de la découverte de l'alphabet arabe jusqu'à la lecture fluide et la mémorisation du Noble Coran, dans un cadre bienveillant et structuré.`,
      en: `${getAcademyName()} is an online Qur'anic academy that guides children and adults, step by step, from discovering the Arabic alphabet to fluent reading and memorization of the Noble Qur'an, in a caring and structured environment.`,
      it: `${getAcademyName()} è un'accademia coranica online che accompagna bambini e adulti, passo dopo passo, dalla scoperta dell'alfabeto arabo fino alla lettura fluente e alla memorizzazione del Nobile Corano, in un ambiente premuroso e strutturato.`,
      ar: `أكاديمية ${getAcademyName()} أكاديميةٌ قرآنية تعليميةٌ عبر الإنترنت، تحتضن الأطفال والكبار على حدٍّ سواء، وتأخذ بأيديهم خطوةً بخطوة؛ بدءًا من تعلّم الحروف العربية، مرورًا بإتقان القراءة الصحيحة المجوَّدة، وصولًا إلى حفظ كتاب الله الكريم، في بيئةٍ تربويةٍ هادئة ومنظَّمة وحاضنة.`,
    },
    subjects: {
...
// =============================================================================
// PRATIQUE DES LETTRES ARABES — page publique séparée, sans création de
// profil ni accès aux données de l'enseignante. À partager directement aux
// élèves pour qu'ils s'entraînent à la prononciation des 28 lettres.
// =============================================================================
function renderPracticeLettersScreen() {
  const idx = STATE.runtime.practiceLetterIndex || 0;
  const letter = LETTERS[idx];

  return `
    <div class="session-header">
      <h2 class="display">🔤 ${t("practiceLettersTitle")}</h2>
    </div>
    <p class="subject-note" style="margin-bottom:16px;text-align:center">${t("practiceLettersIntro")}</p>
    <div style="text-align:center;font-size:13px;color:var(--color-ink-soft);margin-bottom:10px">${idx + 1} / ${LETTERS.length}</div>
    <div class="card" style="text-align:center;padding:40px 20px">
      <div class="arabic" style="font-size:100px;line-height:1;margin-bottom:18px">${letter.char}</div>
      <div style="font-size:22px;font-weight:700;margin-bottom:18px">${letterName(letter)}</div>
      <button class="btn gold block" id="practiceListenBtn" style="max-width:280px;margin:0 auto">🔊 ${t("playAudio")}</button>
      <audio id="practiceAudioPlayer" style="display:none">
        ${audioSourcesFor(letter.id).map((src) => `<source src="${src}">`).join("")}
      </audio>
    </div>
    <div style="display:flex;gap:10px;margin-top:16px">
      <button class="btn secondary" id="practicePrevBtn" style="flex:1" ${idx === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
      <button class="btn gold" id="practiceNextBtn" style="flex:1" ${idx === LETTERS.length - 1 ? "disabled" : ""}>${t("nextBtn")} ›</button>
    </div>
  `;
}
function attachPracticeLettersHandlers() {
  document.getElementById("practiceListenBtn")?.addEventListener("click", () => {
    const audio = document.getElementById("practiceAudioPlayer");
    if (audio) { audio.currentTime = 0; audio.play().catch(() => toast(t("audioMissing"))); }
  });
  document.getElementById("practicePrevBtn")?.addEventListener("click", () => {
    STATE.runtime.practiceLetterIndex = Math.max(0, (STATE.runtime.practiceLetterIndex || 0) - 1);
    renderPublicShell(renderPracticeLettersScreen());
  });
  document.getElementById("practiceNextBtn")?.addEventListener("click", () => {
    STATE.runtime.practiceLetterIndex = Math.min(LETTERS.length - 1, (STATE.runtime.practiceLetterIndex || 0) + 1);
    renderPublicShell(renderPracticeLettersScreen());
  });
}

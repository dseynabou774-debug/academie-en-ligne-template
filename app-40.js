...
function route() {
  ...
  STATE.route = hash;
  const isPublicRoute = hash === "#/discover" || hash === "#/discover/register" || hash.startsWith("#/verify/") || hash === "#/practice-letters";
  ...
  if (hash === "#/practice-letters") return renderPublicShell(renderPracticeLettersScreen());
  ...
  if (hash === "#/teacher/revisions") return renderShell(renderTeacherRevisions());
  ...
  const studentProfileMatch = hash.match(/^#\/student\/(.+)$/);
  if (studentProfileMatch) return renderShell(renderStudentProfile(decodeURIComponent(studentProfileMatch[1])));
...
function renderPublicShell(mainHtml) {
  $app().innerHTML = `
    <div class="topbar">
      <div class="brand">
        ...
        <div class="brand-name">${getAcademyName()}<small>${t("tagline")}</small></div>
      </div>
  ...
  if (STATE.route === "#/practice-letters") attachPracticeLettersHandlers();
...
function renderShell(mainHtml) {
  ...
  if (STATE.route.startsWith("#/student/")) attachStudentProfileHandlers(decodeURIComponent(STATE.route.replace("#/student/", "")));
  if (STATE.route === "#/teacher/students") attachTeacherStudentsHandlers();
  if (STATE.route === "#/teacher/revisions") attachTeacherRevisionsHandlers();
...
// Seuil au-delà duquel un Juz' déjà mémorisé est considéré "à réviser".
const REVISION_THRESHOLD_DAYS = 14;
function daysSinceRevision(record) {
  const ts = record?.lastRevisedAt || record?.completedAt || record?.updatedAt;
  if (!ts) return Infinity;
  return Math.floor((Date.now() - ts) / 86400000);
}
function juzNeedsRevision(record) {
  return !!record?.completed && daysSinceRevision(record) >= REVISION_THRESHOLD_DAYS;
}
async function logJuzRevision(studentId, juzId) {
  const merged = await DB.logJuzRevision(studentId, juzId);
  if (studentId === STATE.activeStudentId) STATE.memorization[juzId] = merged;
  return merged;
}
...
async function toggleJuzDone(juzId) {
  ...
  const extra = { completed: !wasDone };
  if (!wasDone) extra.lastRevisedAt = Date.now();
  const merged = await DB.saveJuzProgress(STATE.activeStudentId, juzId, extra);
...
function renderMemorizationScreen() {
  ...
  const dueCount = JUZ_LIST.filter((j) => juzNeedsRevision(STATE.memorization[j.id])).length;
  const items = JUZ_LIST.map((juz) => {
    const done = isJuzCompleted(juz.id);
    const due = juzNeedsRevision(STATE.memorization[juz.id]);
    return `
      <button class="path-item" data-juz="${juz.id}" style="background:var(--color-surface);box-shadow:var(--shadow-card)">
        <div class="star8 ${done ? "done" : "active"}" style="${due ? "box-shadow:0 0 0 2px var(--color-accent)" : ""}">${done ? "✓" : juz.id}</div>
        <div class="meta">
          <div class="title arabic" style="font-size:15px">${juz.name_ar}</div>
          <div class="sub">${juzLabel(juz)}${due ? ` · <span style="color:var(--color-accent);font-weight:600">🔄 ${t("revisionDueLabel")}</span>` : ""}</div>
        </div>
        <div class="chev">›</div>
      </button>`;
  }).join("");
  ...
  return `
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      ${dueCount > 0 ? `<div style="margin-top:10px;font-size:12.5px;color:var(--color-accent);font-weight:600">🔄 ${t("revisionDueCount", dueCount)}</div>` : ""}
    </div>
...
function openJuzModal(juzId) {
  ...
  const record = STATE.memorization[juzId];
  const done = isJuzCompleted(juzId);
  const due = juzNeedsRevision(record);
  const lastRevisedLabel = record?.lastRevisedAt ? new Date(record.lastRevisedAt).toLocaleDateString() : null;
  openModal(`
    <div class="close-row"><button class="icon-btn" onclick="closeModal()">✕</button></div>
    <div style="text-align:center">
      ...
      ${done ? `
        <div class="card" style="margin-bottom:14px;text-align:left">
          <div style="font-size:13px;color:${due ? "var(--color-accent)" : "var(--color-ink-soft)"};font-weight:${due ? "700" : "400"}">
            ${lastRevisedLabel ? `${t("lastRevisedLabel")} : ${lastRevisedLabel}` : t("neverRevisedLabel")}
            ${due ? ` · 🔄 ${t("revisionDueLabel")}` : ""}
          </div>
        </div>
        <button class="btn gold block" id="logRevisionBtn" style="margin-bottom:10px">🔄 ${t("logRevisionBtn")}</button>
      ` : ""}
  ...
  document.getElementById("logRevisionBtn")?.addEventListener("click", async () => {
    await logJuzRevision(STATE.activeStudentId, juzId);
    closeModal();
...
function renderSettings() {
  return `
    ...
    <div class="section-title">${t("practiceLettersTitle")}</div>
    <p class="subject-note" style="margin-bottom:10px">${t("practiceLettersSettingsNote")}</p>
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">
      <button class="btn secondary block" data-go="#/practice-letters">👀 ${t("previewDiscover")}</button>
      <button class="btn secondary block" id="copyPracticeLinkBtn">🔗 ${t("copyLink")}</button>
    </div>
...
function attachSettingsHandlers() {
  ...
  document.getElementById("copyPracticeLinkBtn")?.addEventListener("click", async () => {
    const url = `${location.origin}${location.pathname}#/practice-letters`;
    try {
...
// Redimensionne une image téléversée (logo, signature) avant de la
// convertir en base64, pour éviter de stocker des photos énormes dans
// IndexedDB. maxDim borne la plus grande dimension (largeur ou hauteur) ;
// le format PNG est conservé pour préserver la transparence des logos.
...
function openPaymentSettingsModal() {
  ...
  // Logo et signature : brouillon local en base64, initialisé avec ce qui
  // est déjà enregistré. Modifié uniquement en mémoire tant que
  // l'enseignante n'appuie pas sur Enregistrer, comme le reste du formulaire.
  let draftLogo = ps.academyLogo || null;
  let draftSignature = ps.academySignature || null;
  ...
  function renderModal() {
    ...
    <div class="section-title" style="margin-top:0">${t("academyLogoLabel")}</div>
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px">
      <div style="width:56px;height:56px;border-radius:10px;background:var(--color-surface-2);display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0">
        ${draftLogo ? `<img src="${draftLogo}" style="width:100%;height:100%;object-fit:contain" />` : `<span style="font-size:22px">🏫</span>`}
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;flex:1">
        <label class="btn secondary" style="text-align:center;cursor:pointer;padding:8px 14px;font-size:12.5px">
          ${t("chooseImageBtn")}
          <input type="file" id="psLogoInput" accept="image/*" style="display:none" />
        </label>
        ${draftLogo ? `<button class="btn secondary" id="psLogoRemoveBtn" style="padding:8px 14px;font-size:12.5px;color:var(--color-danger)">${t("removeImageBtn")}</button>` : ""}
      </div>
    </div>

    <div class="section-title" style="margin-top:0">${t("academySignatureLabel")}</div>
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px">
      <div style="width:56px;height:56px;border-radius:10px;background:var(--color-surface-2);display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0">
        ${draftSignature ? `<img src="${draftSignature}" style="width:100%;height:100%;object-fit:contain" />` : `<span style="font-size:22px">✍️</span>`}
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;flex:1">
        <label class="btn secondary" style="text-align:center;cursor:pointer;padding:8px 14px;font-size:12.5px">
          ${t("chooseImageBtn")}
          <input type="file" id="psSignatureInput" accept="image/*" style="display:none" />
        </label>
        ${draftSignature ? `<button class="btn secondary" id="psSignatureRemoveBtn" style="padding:8px 14px;font-size:12.5px;color:var(--color-danger)">${t("removeImageBtn")}</button>` : ""}
      </div>
    </div>
  ...
  `);
    ...
    document.getElementById("psLogoInput").addEventListener("change", async (e) => {
      const file = e.target.files[0];
      if (!file) return;
      readCurrentTabIntoDraft();
      try {
        draftLogo = await resizeImageForUpload(file, 400);
      } catch (err) {
        toast(t("photoError"));
        return;
      }
    ...
    document.getElementById("psLogoRemoveBtn")?.addEventListener("click", () => {
      readCurrentTabIntoDraft();
      draftLogo = null;
      renderModal();
    });
    document.getElementById("psSignatureInput").addEventListener("change", async (e) => {
      const file = e.target.files[0];
      if (!file) return;
      readCurrentTabIntoDraft();
      try {
        draftSignature = await resizeImageForUpload(file, 400);
      } catch (err) {
        toast(t("photoError"));
        return;
      }
    ...
    document.getElementById("psSignatureRemoveBtn")?.addEventListener("click", () => {
      readCurrentTabIntoDraft();
      draftSignature = null;
      renderModal();
    });
    document.getElementById("confirmPaymentSettingsBtn").addEventListener("click", async () => {
      ...
      Object.assign(STATE.settings.payment, {
        ...
        academyLogo: draftLogo,
        academySignature: draftSignature,
      });
...
function renderPaymentsList() {
  ...
  const statusChips = [
    ...
    ["exempt", billingStatusLabel("exempt")],
  ];
...
function renderPaymentDetail(entityId) {
  ...
  return `
    ...
    <button class="btn secondary block" id="markUpToDateBtn" style="margin-bottom:10px">✅ ${t("markUpToDateBtn")}</button>
    <button class="btn secondary block" id="toggleExemptBtn" style="margin-bottom:20px">${entity.exempt ? "🎗️ " + t("removeExemptBtn") : "🎗️ " + t("markExemptBtn")}</button>
...
function attachPaymentDetailHandlers(entityId) {
  ...
  document.getElementById("toggleExemptBtn")?.addEventListener("click", () => {
    if (entity.exempt) {
      entity.exempt = false;
      saveEntityBilling(entity).then(() => { invalidateBillingCache(); go(`#/payment/${encodeURIComponent(entity.id)}`); });
      return;
    }
    openConfirmModal(t("markExemptTitle"), t("markExemptMsg"), async () => {
      entity.exempt = true;
      await saveEntityBilling(entity);
      invalidateBillingCache();
      closeModal();
      toast(t("markExemptToast"));
      go(`#/payment/${encodeURIComponent(entity.id)}`);
    });
  });
...
function openReceiptLangModal(entity, payment) {
  ...
  const filenameBase = `Recu_${(entity.name || "recu").replace(/\s+/g, "_")}_${payment.coversMonth}`;
  // NOTE DIAGNOSTIC : ces trois boutons n'avaient auparavant aucune
  // capture d'erreur — si drawReceiptCanvas() échouait pour une raison
  // quelconque, rien ne se passait à l'écran (échec totalement silencieux,
  // impossible à diagnostiquer). On affiche maintenant le message d'erreur
  // réel dans un toast, pour identifier la cause exacte si ça se reproduit.
  document.getElementById("rDownloadPngBtn").addEventListener("click", async () => {
    try {
      ...
      console.error("Erreur génération reçu (PNG) :", e);
      toast("Erreur reçu PNG : " + (e && e.message ? e.message : String(e)));
    }
  });
  document.getElementById("rDownloadPdfBtn").addEventListener("click", async () => {
    try {
      ...
      toast("Erreur reçu PDF : " + (e && e.message ? e.message : String(e)));
    }
  });
  document.getElementById("rShareBtn").addEventListener("click", async () => {
    try {
      ...
      console.error("Erreur partage reçu :", e);
      toast("Erreur partage reçu : " + (e && e.message ? e.message : String(e)));
    }
  });
}
...
function renderVerifyResult(type, num, data) {
  const typeLabels = {
    ...
    bulletin: t("verifyTypeBulletin"), receipt: t("verifyTypeReceipt"), report: t("verifyTypeReport"),
  };
  ...
  if (!data) {
    return `
      <div style="max-width:480px;margin:0 auto;padding:24px 20px;text-align:center">
        ...
        <p class="subject-note" style="margin-bottom:8px">${typeLabel} — ${getAcademyName()}</p>
  ...
  return `
    <div style="max-width:480px;margin:0 auto;padding:24px 20px;text-align:center">
      ...
      <p class="subject-note" style="margin-bottom:24px">${typeLabel} — ${getAcademyName()}</p>
  ...
  return `
    <div dir="${isRtl ? "rtl" : "ltr"}" class="${isRtl ? "rtl-content" : ""}">
      ...
      <div class="discover-hero">
        ...
        <h1 class="display">${getAcademyName()}</h1>
...
function attachDiscoverScreenHandlers() {
  ...
  document.getElementById("discoverShareBtn")?.addEventListener("click", () => {
    ...
    const text = `${getAcademyName()} — ${dt("heroTagline", lang)}\n${shareUrl}`;
...
// Petites horloges pour les pays où se trouvent les élèves (utile pour
// coordonner les créneaux avec les familles à l'international). Heure
// calculée au moment de l'affichage — se met à jour à chaque navigation.
function renderWorldClocksWidget() {
  const cities = [
    { flag: "🇸🇳", label: "Sénégal", tz: "Africa/Dakar" },
    { flag: "🇫🇷", label: "France", tz: "Europe/Paris" },
    { flag: "🇮🇹", label: "Italie", tz: "Europe/Rome" },
    { flag: "🇺🇸", label: "États-Unis (Est)", tz: "America/New_York" },
  ];
  return `
    <div class="card" style="margin-bottom:16px">
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;text-align:center">
        ${cities.map((c) => {
          const time = new Intl.DateTimeFormat("fr-FR", { timeZone: c.tz, hour: "2-digit", minute: "2-digit" }).format(new Date());
          return `
            <div>
              <div style="font-size:18px">${c.flag}</div>
              <div style="font-size:15px;font-weight:700">${time}</div>
              <div style="font-size:10px;color:var(--color-ink-soft)">${c.label}</div>
            </div>
          `;
        }).join("")}
      </div>
...
function openAttendanceModal(occ) {
  ...
  document.getElementById("attSaveBtn").addEventListener("click", async () => {
    ...
    if (occ.isAdHoc) {
      ...
      log.studentIds = occ.studentIds || (occ.studentId ? [occ.studentId] : []);
      await DB.saveCourseLog(log);
    }
    closeModal();
    renderShell(renderScheduleScreen());

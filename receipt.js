...
// Charge le logo et la signature à afficher sur le reçu. Priorité
// systématique aux images téléversées par l'enseignante depuis Réglages >
// Informations de l'académie (STATE.settings.payment.academyLogo /
// .academySignature, en base64) ; repli sur les fichiers statiques du
// dépôt (logo-official.png / signature-teacher.png) s'ils existent
// encore, sinon aucune image (le reçu reste lisible sans logo/signature).
// Le cache mémoire ne concerne que le repli statique : les images
// téléversées sont relues depuis STATE à chaque appel, pour refléter
// immédiatement un changement fait dans les Réglages.
let _receiptStaticLogoImg = null, _receiptStaticSignatureImg = null;
function loadReceiptImages() {
  const uploadedLogo = STATE.settings?.payment?.academyLogo;
  const uploadedSignature = STATE.settings?.payment?.academySignature;
  return Promise.all([
    uploadedLogo
      ? new Promise((resolve) => {
          const img = new Image();
      ...
      : new Promise((resolve) => {
          if (_receiptStaticLogoImg) return resolve(_receiptStaticLogoImg);
          const img = new Image();
          img.onload = () => resolve(img);
          img.onerror = () => resolve(null);
          img.src = "logo-official.png";
          _receiptStaticLogoImg = img;
        }),
    uploadedSignature
      ? new Promise((resolve) => {
          const img = new Image();
      ...
      : new Promise((resolve) => {
          if (_receiptStaticSignatureImg) return resolve(_receiptStaticSignatureImg);
          const img = new Image();
          img.onload = () => resolve(img);
          img.onerror = () => resolve(null);
          img.src = "signature-teacher.png";
          _receiptStaticSignatureImg = img;
        }),
  ]);
}
...
async function drawReceiptCanvas(entity, payment, lang) {
  ...
  const verifyUrl = buildVerifyUrl("receipt", payment.receiptNumber);
  ...
  const academyName = (typeof getAcademyName === "function") ? getAcademyName() : (ps.academyName || "Mon École");
  ...
  const quote = (ps.receiptQuote && (ps.receiptQuote[lang] || ps.receiptQuote.fr)) || "";
  const quoteAuthor = (ps.receiptQuoteAuthor && (ps.receiptQuoteAuthor[lang] || ps.receiptQuoteAuthor.fr)) || "";
  if (quote) {
  ...
  const thanksText = (ps.receiptThanks && (ps.receiptThanks[lang] || ps.receiptThanks.fr)) || "";
  const wishText = (ps.receiptWish && (ps.receiptWish[lang] || ps.receiptWish.fr)) || "";
  ctx.textAlign = "center";
  if (thanksText) {
    ctx.font = "bold 19px Georgia, serif"; ctx.fillStyle = "#0B6E4F";
    ctx.fillText(thanksText, W / 2, fy);
    fy += 32;
  }
  if (wishText) {
    ctx.font = "15px Georgia, serif"; ctx.fillStyle = "#6B7469";
    const wishLines = wrapCanvasText(ctx, wishText, W / 2, fy, W - 220, 22);
    fy += wishLines * 22 + 20;
  }

...
// -----------------------------------------------------------------------
// Nom de l'académie — lu dynamiquement depuis les Réglages (STATE.settings
// .payment.academyName), pour que ce modèle générique affiche le nom de
// CHAQUE académie qui l'utilise, sans jamais avoir à toucher au code.
// Repli sur un nom générique si l'enseignante n'a pas encore renseigné le
// sien dans Réglages > Informations de l'académie.
// -----------------------------------------------------------------------
function getAcademyName() {
  const name = (typeof STATE !== "undefined" && STATE.settings && STATE.settings.payment && STATE.settings.payment.academyName) || "";
  return name.trim() || "Mon École";
}

// Découpe le nom de l'académie en 2-3 lignes courtes pour l'affichage dans
// le sceau doré circulaire (qui n'a la place que pour quelques mots par
// ligne). Répartit les mots équitablement plutôt que de tout mettre sur
// une seule ligne trop longue.
function getAcademySealLines(name) {
  const words = (name || getAcademyName()).split(/\s+/).filter(Boolean);
  if (words.length <= 1) return words.length ? words : ["École"];
...
function generateCertNumber() {
  ...
  return `CERT-${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}-${rand}`;
}
...
// -----------------------------------------------------------------------
// Assets partagés (logo, signature) — chaque académie utilisant ce modèle
// peut téléverser son PROPRE logo et sa PROPRE signature depuis
// Réglages > Informations de l'académie (voir openPaymentSettingsModal
// dans app.js), stockés en base64 dans STATE.settings.payment.academyLogo
// / .academySignature. Priorité systématique à ces images téléversées ;
// repli sur un fichier statique du dépôt (logo-official.png /
// signature-teacher.png) UNIQUEMENT s'il existe encore (compatibilité
// avec d'anciennes installations qui avaient ces fichiers), sinon aucune
// image n'est affichée — les diplômes restent parfaitement lisibles sans
// logo ni signature, ce ne sont que des ornements optionnels.
// -----------------------------------------------------------------------
let _staticLogoDataUrlCache = null;
async function getLogoDataUrl() {
  const uploaded = typeof STATE !== "undefined" && STATE.settings?.payment?.academyLogo;
  if (uploaded) return uploaded;
  if (_staticLogoDataUrlCache !== null) return _staticLogoDataUrlCache;
  try {
    const res = await fetch("logo-official.png");
    if (!res.ok) throw new Error("no logo");
    const blob = await res.blob();
    _staticLogoDataUrlCache = await new Promise((resolve) => {
      const reader = new FileReader();
      reader.onloadend = () => resolve(reader.result);
      reader.readAsDataURL(blob);
    });
  } catch (e) {
    _staticLogoDataUrlCache = false;
  }
  return _staticLogoDataUrlCache || null;
}

let _staticSignatureDataUrlCache = null;
async function getSignatureDataUrl() {
  const uploaded = typeof STATE !== "undefined" && STATE.settings?.payment?.academySignature;
  if (uploaded) return uploaded;
  if (_staticSignatureDataUrlCache !== null) return _staticSignatureDataUrlCache;
  try {
    const res = await fetch("signature-teacher.png");
    if (!res.ok) throw new Error("no signature");
    const blob = await res.blob();
    _staticSignatureDataUrlCache = await new Promise((resolve) => {
      const reader = new FileReader();
      reader.onloadend = () => resolve(reader.result);
      reader.readAsDataURL(blob);
    });
  } catch (e) {
    _staticSignatureDataUrlCache = false;
  }
  return _staticSignatureDataUrlCache;
}
...
async function buildDiplomaImage({ topTitle, subtitle, ofLabel, studentName, forLabel, achievementTitle, bodyText, deliveredOnLabel, issuedAtLabel, certNumberLabel, certNumber, signatureRoleLabel, verifyLabel, qrDataUrl }) {
  ...
  ctx.fillText(getAcademyName(), W / 2, y);
  y += 90;
  ...
  ctx.fillText(getAcademyName(), W - 280, footY - 46);
...
async function buildDiplomaPdf(opts) {
  ...
  doc.setFont("times", "bold");
  doc.setFontSize(13);
  doc.text(getAcademyName(), W / 2, 42, { align: "center" });
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.3);
  ...
  drawSeal(doc, W / 2, footY - 2, 13, getAcademySealLines());

  const sigX = W - 78;
  doc.setTextColor(...INK);
  doc.setFont("times", "normal");
  doc.setFontSize(9.5);
  doc.text(getAcademyName(), sigX, footY - 12, { align: "center" });
...
async function buildKhatmDiplomaPdf(opts) {
  ...
  doc.setFont("times", "bold");
  doc.setFontSize(14);
  doc.text(getAcademyName(), W / 2, 53, { align: "center" });
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.4);
  ...
  doc.setFont("times", "normal");
  doc.setFontSize(9.5);
  doc.text(getAcademyName(), sigX, footY - 16, { align: "center" });
...
const BULLETIN_LABELS = {
  fr: {
    ...
    signature: (name) => `${name} — Signature de l'enseignante`,
  ...
  en: {
    ...
    signature: (name) => `${name} — Teacher's signature`,
  ...
  it: {
    ...
    signature: (name) => `${name} — Firma dell'insegnante`,
...
async function buildBulletinPdf(opts) {
  ...
  doc.setFont("times", "bold");
  doc.setFontSize(10.5);
  doc.text(getAcademyName(), W / 2, 30, { align: "center" });
  ...
  doc.setFontSize(9);
  doc.setTextColor(...ink);
  doc.text(L.signature(getAcademyName()), W / 2, H - 26, { align: "center" });
  const signatureDataUrl = await getSignatureDataUrl();
  if (signatureDataUrl) {
    const sigW = 30;
    const sigImgProps = doc.getImageProperties(signatureDataUrl);
...
async function buildBulletinPng(opts) {
  ...
  ctx.fillText(L.signature(getAcademyName()), W / 2, footY);
  if (signatureImg) {
 ...
 */
// =============================================================================
// RELEVÉ DÉTAILLÉ — compile le contenu réellement étudié (sourates, alphabet,
// Athkar, Fiqh, Tafsir, Sira) sur une période choisie, pour l'envoi
// périodique aux parents. Document distinct du bulletin d'évaluation (qui
// reste inchangé). La section Alphabet a été ajoutée ici : elle manquait
// entièrement (le module de suivi existait dans la fiche élève mais
// n'était jamais lu par ce générateur de relevé).
// =============================================================================
const REPORT_LABELS = {
  fr: {
    title: "RELEVÉ DÉTAILLÉ", period: (from, to) => `Période du ${from} au ${to}`,
    student: "Élève", generatedOn: "Établi le", reportNumber: "N° relevé",
    quranSection: "📖 Coran — Sourates étudiées", alphabetSection: "🔤 Alphabet arabe travaillé", athkarSection: "🌿 Athkar appris",
    fiqhSection: "📚 Thèmes de Fiqh étudiés", tafsirSection: "📖 Tafsir étudié", siraSection: "🌙 Sira étudiée",
    checklistSection: "📗 Sourates déjà acquises (Juz Amma)",
    noEntries: "Aucune entrée enregistrée sur cette période.",
    noChecklist: "Aucune sourate cochée comme déjà acquise à ce jour.",
    signature: (name) => `${name} — Signature de l'enseignante`, verify: "Vérification du relevé",
    versesLabel: "versets", lastLetterLabel: "dernière lettre",
  },
  en: {
    title: "DETAILED REPORT", period: (from, to) => `Period from ${from} to ${to}`,
    student: "Student", generatedOn: "Issued on", reportNumber: "Report No.",
    quranSection: "📖 Qur'an — Surahs studied", alphabetSection: "🔤 Arabic alphabet worked on", athkarSection: "🌿 Athkar learned",
    fiqhSection: "📚 Fiqh themes studied", tafsirSection: "📖 Tafsir studied", siraSection: "🌙 Sira studied",
    checklistSection: "📗 Surahs already mastered (Juz Amma)",
    noEntries: "No entries recorded for this period.",
    noChecklist: "No surah marked as already mastered so far.",
    signature: (name) => `${name} — Teacher's signature`, verify: "Report verification",
    versesLabel: "verses", lastLetterLabel: "last letter",
  },
  it: {
    ...
    quranSection: "📖 Corano — Sure studiate", alphabetSection: "🔤 Alfabeto arabo trattato", athkarSection: "🌿 Athkar appresi",
    fiqhSection: "📚 Temi di Fiqh studiati", tafsirSection: "📖 Tafsir studiato", siraSection: "🌙 Sira studiata",
    checklistSection: "📗 Sure già acquisite (Juz Amma)",
    noEntries: "Nessuna voce registrata in questo periodo.",
    noChecklist: "Nessuna sura contrassegnata come già acquisita finora.",
    signature: (name) => `${name} — Firma dell'insegnante`, verify: "Verifica del resoconto",
    versesLabel: "versetti", lastLetterLabel: "ultima lettera",
  },
};
...
async function buildDetailedReportCanvas(opts) {
  ...
  const sections = [
    { label: L.quranSection, items: opts.quranItems, empty: L.noEntries },
    { label: L.alphabetSection, items: opts.alphabetItems || [], empty: L.noEntries },
    { label: L.checklistSection, items: opts.checklistItems || [], empty: L.noChecklist },
    { label: L.athkarSection, items: opts.athkarItems, empty: L.noEntries },
    { label: L.fiqhSection, items: opts.fiqhItems, empty: L.noEntries },
    { label: L.tafsirSection, items: opts.tafsirItems, empty: L.noEntries },
    { label: L.siraSection, items: opts.siraItems, empty: L.noEntries },
  ];
  ...
  sections.forEach((sec) => {
    ...
    if (sec.items.length === 0) {
      ...
      ctx.fillText(sec.empty || L.noEntries, 50, y);
      y += 28;
    } else {
...
async function generateDetailedReport(studentId, fromISO, toISO, lang) {
  ...
  const [quranEntries, topicEntries, surahChecklist, alphabetEntries] = await Promise.all([
  ...
  // Suivi de l'alphabet arabe (nouvelle section) : lettres travaillées +
  // dernière lettre atteinte le cas échéant, filtré sur la même période
  // que les autres suivis datés.
  const alphabetItems = alphabetEntries.filter((e) => inRange(e.date)).sort((a, b) => a.date.localeCompare(b.date))
    .map((e) => `${(e.letters || []).join(" ")}${e.lastLetter ? ` (${L.lastLetterLabel}: ${e.lastLetter})` : ""} — ${new Date(e.date).toLocaleDateString()}`);

  // Checklist "sourates déjà acquises" : indépendante de la période choisie
  // (pas de date associée), affichée en l'état actuel dans son propre bloc.
  const acquiredIds = new Set((surahChecklist && surahChecklist.surahIds) || []);
  const checklistItems = (typeof SHORT_SURAHS !== "undefined" ? SHORT_SURAHS : [])
    .filter((s) => acquiredIds.has(s.id))
    .map((s) => `${s.name_ar} — ${s.name_fr}`);

  const reportNumber = generateCertNumber().replace("CERT-", "REP-");
  const canvas = await buildDetailedReportCanvas({
    ...
    quranItems, alphabetItems, checklistItems, athkarItems: byTopic("athkar"), fiqhItems: byTopic("fiqh"), tafsirItems: byTopic("tafsir"), siraItems: byTopic("sira"),
  });
...
async function generateBulletin(type, sessionId, lang, comment, observations) {
  ...
  const bulletinNumber = generateCertNumber().replace("CERT-", "BULL-");
